...
public class CMEntity extends StandEntity {
    ...
    private void handleBuffs() {
        ...
        if (power.getStamina() < 10) {
            //level.playSound(null, this.blockPosition(), InitSounds.CM_UNSUMMON_SOUND.get(), SoundCategory.PLAYERS, 1, 1);
            power.toggleSummon();
            return;
        }

        float energyRatio = power.getStamina() / power.getMaxStamina();
        float particlesPerTick = energyRatio * 1f;
        IParticleData particleType = InitParticles.HAMON_AURA_PURPLE.get();

        user.addEffect(new EffectInstance(InitEffects.CM_SHIFT.get(), 100, 4, false, false, true));
        ...
        for (Entity entity : MCUtil.entitiesAround(
            ...
            if (!(entity instanceof ProjectileEntity) || (entity instanceof LivingEntity)) return;
